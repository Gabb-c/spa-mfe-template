...
export const actFigure = () => {
  const canvas = document.querySelector('canvas') ? document.querySelector('canvas') : undefined;
  ...
  const colorHead = 'rgba(7, 8, 105, 0.82)';
  const colorBody = 'rgba(7,10,136,.86)';
  const colorEyes = 'rgba(205, 219, 211, 0.82)';
  const colorShadow = 'rgba(50,30,36,.15)';
  ...
  const connectTentacles = () => {
    ...
    const y = squid.y + squid.bodyHeight;
    ...
    tentacles.forEach((tentacle) => {
      ...
      connector.x = squid.x + Math.cos(angleDiff + squid.angle) * hd;
      connector.y = squid.y + Math.sin(angleDiff + squid.angle) * hd;
  ...
  const drawParticles = () => {
    particles.forEach((p) => {
      ...
      ctx.beginPath();
      ctx.fillStyle = `rgba(25, 30, 36, ${p.life})`;
  ...
  const onPointerMove = (e) => {
    const target = e.touches && e.touches.length > 0 ? e.touches[0] : e;
